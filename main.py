...
class ProductPage:
    ...
    def __init__(self, header, description, picture, footer=None) -> None:
...
class TemplateContext:
    ...
    def __tile(self, row: tuple): return {
        'home' : lambda: { 'href':f"view?what=news&id={row[0]}", 'text':row[1], 'picture':row[3] },
        'info' : lambda: { 'href':f"view?what=info&id={row[0]}", 'text':row[1], 'picture':row[3] },
        'services' : lambda: { 'href':f"view?what=service&id={row[0]}", 'text':row[1], 'price':row[3], 'picture':row[4] },
        'recommended' : lambda: { 'href':f"view?what={row[0]}&id={row[1]}", 'text':row[2], 'picture':row[3] },
        'guide' : lambda: { 'href':f"guide-grouped?category={row[1]}", 'text':row[2], 'picture':row[3] },
        'menu' : lambda: { 'href':f"view?what=menu&id={row[0]}", 'text':row[1], 'price':row[3], 'picture':row[5] },
        'apps' : lambda: { 'href':row[2], 'text':row[1], 'picture':row[3] },
        'guide-grouped' : lambda: { 'href':f"view?what=guide&id={row[0]}", 'text':row[1], 'picture':row[5] }
    ...
    def __get_content(self, target: str, id: int): 
        data = self.__func.get(target)(id); return {
            'news' : lambda: ProductPage(header=data[0], description=data[1], picture=data[2]),
            'info' : lambda: ProductPage(header=data[0], description=data[1], picture=data[2]),
            'service' : lambda: ProductPage(header=data[0], description=data[1], picture=data[3]),
            'guide' : lambda: ProductPage(header=data[0], description=data[1], footer=data[2], picture=data[3]),
            'menu' : lambda: ProductPage(header=data[0], description=data[1], picture=data[3])
        }.get(target)().to_dict()
    ...
    nav_bar_content: list = [
        ...
        { 'id' : 'menu', 'text' : 'Меню' },
        { 'id' : 'applications', 'text' : 'Приложения' },
    ]
    ...
    def tile_content(self): return {
        ...
        'apps' : lambda: [dict(self.__tile(row)) for row in get_all_applications()],
...
def _(request: Request): return templates.TemplateResponse('home.html', TemplateContext(
    request=request,
...
def _(request: Request): return templates.TemplateResponse('info.html', TemplateContext(
    request=request,
    current_page='info',
).to_dict())
...
def _(request: Request): return templates.TemplateResponse('tiles.html', TemplateContext(
    request=request,
...
@app.get('/{device_id}/applications', response_class=HTMLResponse)
def _(request: Request): return templates.TemplateResponse('tiles.html', TemplateContext(
    request=request,
    current_page='apps',
    title='Приложения'
...
def _(request: Request): return templates.TemplateResponse('product-page.html', TemplateContext(
    request=request,
    solid_background=True,
